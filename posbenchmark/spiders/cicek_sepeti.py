...
class CiceksepetiSpider(scrapy.Spider):
    name = "ciceksepeti"
    allowed_domains = ["ciceksepeti.com"]
    start_urls = ["https://ciceksepeti.com"]
    ...
    custom_settings = {
        ...
        # },
        "ROBOTSTXT_OBEY": True,
        "DOWNLOAD_DELAY": 10,  # 2-second delay between requests
    ...
    def get_search_url(self, base_url, model):
        return f"{base_url}/arama?query={model.replace(' ', '%20')}&qt={model.replace(' ', '%20')}"
    ...
    async def parse(self, response):
        ...
        for product in response.xpath("//div[@class = 'listing__products']/a"):
            ...
            if subtitle is not None:
                ...
                subtitle = ""
            title = f"{product.xpath(".//div[@data-cs-pb-name-wrapper = 'true']/span/text()").get()}" # Adjust the selector for the title
            price_text = f"{product.xpath(".//span[@data-cs-pb-price-text='true']/text()").get()}"  # Adjust the selector for the price
            self.logger.info(title.lower())
            discounted_price_text = f"{product.xpath(".//span[@data-cs-pb-price-text='true']/text()").get()}"

            if discounted_price_text is not None:
                self.logger.info(f"bağırma lan: {discounted_price_text}")
                price_text = discounted_price_text.replace("Sepette ", "")
                print(price_text)
    ...
    def closed(self, reason):
        ...
        try:
            df = pd.read_csv("ciceksepeti_prices.csv")
            ...
            self.logger.warning("ciceksepeti_prices.csv not found. No pivot table created.")
