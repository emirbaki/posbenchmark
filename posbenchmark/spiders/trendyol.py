...
class TrendyolPriceSpider(scrapy.Spider):
    name = "trendyol_price"
    ...
    def get_search_url(self, base_url, model):
       return f"{base_url}/sr?q={model.replace(' ', '%20')}&qt={model.replace(' ', '%20')}&st={model.replace(' ', '%20')}"
        # return f"{base_url}/sr?q={model.replace(' ', '%20')}"
    ...
    async def parse(self, response):
        ...
        for product in response.css(".p-card-chldrn-cntnr.card-border"):
            ...
            # if subtitle:
            #     self.logger.info(f"cart curt{subtitle.lower()}")
            ...
            self.logger.info(f'niye true geliyorum bilmiyorum: {flagged_as_bad}')
    ...
    def closed(self, reason):
        ...
        try:
            df = pd.read_csv("trendyol_prices.csv")
            ...
            self.logger.warning("trendyol_prices.csv not found. No pivot table created.")
