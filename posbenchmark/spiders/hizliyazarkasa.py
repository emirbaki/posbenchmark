    ...
    async def parse(self, response):
        ...
        for model in self.pos_models:
            for product in response.xpath("//div[@class='container th-container']/div/div/div[@class = 'project-grid']"):
                title = str(product.css('div.box-content').xpath('.//h3/a/text()').get()) # Adjust the selector for the title
                price_text = str(product.css('div.box-content').css('span.price::text').get())  # Adjust the selector for the price
